...
{
	int n=g->NumVertices;
	bool *visited=(bool*)malloc(sizeof(bool)*n);//创建一个辅助数组，遍历过的元素设置为true
	assert(visited != NULL);
	for(int i=0;i<n;++i)
	{
		visited[i]=false;//初值设置为false
	}
...
void DFS(GraphLnk *g, int v, bool visited[])
{
	...
	visited[v]=true;//访问过后设置为true
	int w=getFirstNeighbor(g,getVertexValue(g,v));//获取v的第一个邻接顶点
	while(w != -1)
	{
		if(!visited[w])
		{
			DFS(g,w,visited);//递归遍历
		}
		w=getNextNeighbor(g,getVertexValue(g,v),getVertexValue(g,w));//获取v的下一个邻接顶点
	}
}
//最广路径排序(需要借助一个队列)
void BFS(GraphLnk *g,T vertex)
{
	...
	EnQueue(&q,v);//将第一个顶点的下标放入队列
	int w;
	while(!Empty(&q))
	{
		GetHead(&q,&v);
		DeQueue(&q);//将第一个顶点的下标出队

		w=getFirstNeighbor(g,getVertexValue(g,v));//获取v的第一个邻接顶点
		while(w!=-1)
		{
			if(!visited[w])//w未遍历时
			{
				printf("%c-->",getVertexValue(g,w));
				visited[w]=true;//访问后设置为true
				EnQueue(&q,w);//将w入队
			}
			w=getNextNeighbor(g,getVertexValue(g,v),getVertexValue(g,w));//获取v的下一个邻接顶点
		}
